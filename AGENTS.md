...
- Before production deploys, the integration thread must promote the approved candidate onto `main`.
- Production deploys must start from a clean `main` worktree. Feature threads must not deploy production directly.
